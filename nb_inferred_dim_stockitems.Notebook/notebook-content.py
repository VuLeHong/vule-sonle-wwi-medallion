...
# ============================================================
# DEDICATED GOLD NOTEBOOK — INFERRED DIM_STOCKITEMS
# Source fact staging: Silver.Sales_Orders + Silver.Sales_OrderLines
# Target dimension : Gold.Dim_StockItems
# Purpose:
#   - Run before loading Fact_Sales
#   - Check business key from fact source
#   - If key does not exist in dimension, insert inferred row
...
DIM_SOURCE_OBJECT = "Warehouse_StockItems"
...
print("=" * 80)
print("[START] Inferred Dim_StockItems load started")
...
try:
    ...
    audit_ts_expr = F.to_timestamp(F.col("ol.audit_ts")).alias("audit_ts")
    source_id_expr = F.col("ol.source_id").alias("source_id")
    ...
    if source_key_count == 0:
        ...
        print("=" * 80)
        print("[SUCCESS] Inferred Dim_StockItems load completed - No source business keys")
...
try:
    ...
    if missing_key_count == 0:
        ...
        print("=" * 80)
        print("[SUCCESS] Inferred Dim_StockItems load completed - No inferred rows to insert")
...
try:
    ...
    print("=" * 80)
    print("[SUCCESS] Inferred Dim_StockItems load completed")
    print(f"[TARGET_OBJECT] {TARGET_OBJECT}")
    print(f"[BUSINESS_KEYS] {business_keys}")
    print(f"[SOURCE_KEYS] {source_key_count}")
    print(f"[MISSING_KEYS] {missing_key_count}")
