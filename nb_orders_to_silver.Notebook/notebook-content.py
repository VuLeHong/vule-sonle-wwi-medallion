...
print("=" * 80)
print("[START] Silver delta load started")
...
try:
    ...
    if not config_row:
        raise RuntimeError(
            f"[ERROR] Không tìm thấy config cho '{TARGET_OBJECT}' trong {CONFIG_TABLE}"
        )
    ...
    print(f"[CONFIG] Load type     : {load_type}")
...
try:
    ...
    if bronze_raw_df.isEmpty():
        ...
        print("=" * 80)
        print("[SUCCESS] Silver delta load completed - No new Bronze data")
        ...
        latest_audit_ts = (
            bronze_raw_df
            .select(F.max("audit_ts").alias("latest_audit_ts"))
            .collect()[0]["latest_audit_ts"]
        )

        if latest_audit_ts is None:
            raise RuntimeError(
                f"[ERROR] Không tìm thấy audit_ts hợp lệ trong Bronze của {TARGET_OBJECT}"
            )
...
# ============================================================
# 4. LẤY LATEST SNAPSHOT TỪ BRONZE + CLEANING
# ============================================================

try:
    df_bronze_delta = (
        bronze_raw_df
        .filter(F.col("audit_ts") == F.lit(latest_audit_ts))
    )
    ...
    if df_bronze_delta.isEmpty():
        raise RuntimeError(
            f"[ERROR] Latest Bronze snapshot của {TARGET_OBJECT} bị trống"
        )

    bronze_delta_count = df_bronze_delta.count()

    print(f"[INFO] Số dòng trong latest Bronze snapshot: {bronze_delta_count}")

except Exception as e:
    print(f"[FAILED] Prepare latest Bronze snapshot failed: {str(e)}")
    raise
...
try:
    ...
    if not spark.catalog.tableExists(silver_table):
        print(f"[FIRST-RUN] Bảng {silver_table} chưa tồn tại. Insert toàn bộ staged data.")
        ...
        df_changed = (
            df_staged
            .join(
                df_silver_active.withColumnRenamed("row_hash", "_silver_row_hash"),
                on="hash_key",
                how="inner"
            )
...
try:
    ...
    if rows_to_insert > 0:
        ...
        print(f"[SUCCESS] Đã append {rows_to_insert} dòng vào {silver_table}")
    else:
...
try:
    ...
    print(f"[FAILED] Append Silver watermark failed: {str(e)}")
    raise
...
try:
    print("=" * 80)
    print("[SUCCESS] Silver delta load completed")
    print(f"[TARGET_OBJECT] {TARGET_OBJECT}")
    print(f"[SILVER_TABLE] {silver_table}")
