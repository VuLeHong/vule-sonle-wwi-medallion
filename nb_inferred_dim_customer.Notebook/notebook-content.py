...
# ============================================================
# DEDICATED GOLD NOTEBOOK — INFERRED DIM_CUSTOMERS
# Source fact staging: Silver.Sales_Orders + Silver.Sales_OrderLines
# Target dimension : Gold.Dim_Customer
# Purpose:
#   - Run before loading Fact_Sales
#   - Check CustomerID from fact source
#   - If CustomerID does not exist in Dim_Customer, insert inferred row
...
DIM_SOURCE_OBJECT = "Sales_Customers"
...
#
# Với Dim_Customer:
#   business_key trong config thường là CustomerID
#   CustomerID nằm ở Sales_Orders
# ============================================================
...
print("=" * 80)
print("[START] Inferred Dim_Customer load started")
...
#    Follow template:
#    - read source fact staging
#    - trim(coalesce(key, ''))
#    - row_number partition by business key
# ============================================================
...
try:
    ...
    # Với Dim_Customer, CustomerID lấy từ Orders,
    ...
    if source_key_count == 0:
        ...
        print("=" * 80)
        print("[SUCCESS] Inferred Dim_Customer load completed - No source business keys")
...
try:
    ...
    if missing_key_count == 0:
        ...
        print("=" * 80)
        print("[SUCCESS] Inferred Dim_Customer load completed - No inferred rows to insert")
...
try:
    df_inferred = (
        ...
        .withColumn("scd_from", F.col("audit_ts"))
...
try:
    ...
    print("=" * 80)
    print("[SUCCESS] Inferred Dim_Customer load completed")
    print(f"[TARGET_OBJECT] {TARGET_OBJECT}")
    print(f"[BUSINESS_KEYS] {business_keys}")
    print(f"[SOURCE_KEYS] {source_key_count}")
    print(f"[MISSING_KEYS] {missing_key_count}")
